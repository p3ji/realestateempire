...
export const useGameState = () => {
  ...
  // Keep ref in sync so intervals can read latest state without re-creating
  useEffect(() => {
  ...
  // Start new game
  const startNewGame = (playerName: string, archetypeKey: string) => {
  ...
  const purchaseProperty = (propertyId: string, useMortgage: boolean, downPayment: number) => {
    setGameState(prev => {
      ...
      player.cash -= cost;
      player.portfolio.push(propertyId);
  ...
  const sellProperty = (propertyId: string) => {
    setGameState(prev => {
      ...
      const tx: Transaction = {
        ...
        date: prev.gameDate,
        description: `Sold ${property.address} for ${property.marketValue} (Paid off ${Math.round(loanBalance)} loan)`,
        amount: cashReceived,
        category: 'sale',
      };
  ...
  useEffect(() => {
    ...
    const runTick = () => {
      ...
      if (!current || current.speed === 0) return;
      ...
      let properties = current.properties.map(p => {
        ...
        let currentMarketValue = p.marketValue * (1 + dailyAppreciation + dailyNoise);
        // Condition affects market value (bad condition drops value up to 20%)
        const conditionModifier = 0.8 + (newCondition / 100) * 0.2;
        currentMarketValue = p.basePrice * (currentMarketValue / p.basePrice) * conditionModifier;
      ...
      if (isMonthEnd) {
        ...
        properties = properties.map(p => {
          ...
          const conditionPenalty = (100 - p.condition) * 0.01; // poor condition = high maintenance
        ...
        player.netWorthHistory.push({
          date: nextDate,
          value: totalEquity,
        });

        // Limit net worth history to last 24 points to avoid massive saves
        if (player.netWorthHistory.length > 24) {
          player.netWorthHistory.shift();
        }
      }
